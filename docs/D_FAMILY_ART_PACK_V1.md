...
- 所有角色主体、Projectile、命中特效与状态特效必须是**真实 Alpha 通道透明 PNG**；棋盘格、白底、灰底、背景、地面和投影均不得烘焙进图片。
- 三人各自的日常衣服是身份识别的一部分，必须保留。升级只能在原服装外叠加护臂、能源核心、背包、悬浮模块、冰晶或粒子等可拆卸装备；不得换制服、盔甲外套、披风或陌生服装。
- 爸爸日常服装锁定为黑色上衣与**米色长裤**；此前黑裤候选稿仅作废稿，不可作为正式资源。
- 禁止“天眼”、额外眼睛、额头能量眼、异形眉眼、额头装置或任何改变真实五官识别的设计。

## 强制右向战斗 Sprite 规范（优先级最高）

适用于呆、逗、小R的全部战场角色主体；不满足任一条的图片不得入库。

1. 敌人从画面右侧接近，单位必须明确面向右侧敌人。
2. 头部、眼睛、胸口、肩膀、攻击手与脚尖均指向右侧；允许极轻微的右前 3/4 角度，但禁止正面、回头或向镜头展示。
3. 人物识别依靠右侧脸轮廓、发型、衣服和小幅侧脸完成，不能为了展示脸而扭转躯干。
4. 脊柱挺直、胸口打开、肩膀后展；双脚前后小幅错开，重心稳定。禁止驼背、大弓步、武术姿势和海报摆拍。
5. 攻击手已对准右侧，火核 / 冰晶 / 雷核必须位于攻击方向前方，表现“即将发射”，而非托在掌心展示。
6. 非攻击手收近躯干并保持戒备；单位主体不横向展开过宽，适合放入单个塔防格。
7. 初级角色单体建议占格：宽度 70%–80%，高度约 85%；脚底锚点统一，人物本体不越格，只有 Projectile 与攻击特效可以延伸。
8. 角色画面语义必须一眼成立：**怪从右边来，他正在看右边，并准备攻击右边。**
