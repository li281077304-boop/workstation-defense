...
describe('confirmed board and turn rules', () => {
  ...
  it('keeps the highest birth value among captures; equal does not merge', () => { const s=emptyState();s.birthSlot=8;s.plants[0][0]=plant(4);s.rewardBalls.push({id:'a',row:0,col:2,value:4},{id:'b',row:0,col:3,value:16});const m=new TurnManager(s,DEFAULT_RULES);m.perform({from:{row:0,col:0},to:{row:0,col:1}});expect(m.state.birthSlot).toBe(16); });
  ...
  it('creates the confirmed projectile count and damage through the 8192 visual cap', () => { const m=new TurnManager(emptyState(),DEFAULT_RULES);expect([1,2,4,32,256,512,8192].map(value => m.createProjectiles(value,0).map(p=>p.remainingDamage))).toEqual([[1],[1,1],[1,1,1,1],[8,8,8,8],[64,64,64,64],[128,128,128,128],[2048,2048,2048,2048]]); });
  ...
  it('naturally spawns reward balls from the configured pool when enabled', () => { const s=emptyState();s.birthSlot=2;s.plants[0][0]=plant(2);const m=new TurnManager(s,{...DEFAULT_RULES,rewardSpawning:true,rewardSpawnChance:1},()=>0);m.perform({from:'birth',to:{row:0,col:0}});expect(m.state.rewardBalls.length).toBeGreaterThan(0);expect(m.state.rewardBalls[0].value).toBeLessThanOrEqual(8);expect(m.state.rewardBalls[0].col).toBeGreaterThanOrEqual(3); });
  it('captures a reward ball then destroys the whole projectile and its remaining damage', () => { const s=emptyState();s.enemies.push({id:'enemy',row:0,col:3,width:1,height:1,hp:99,maxHp:99});s.rewardBalls.push({id:'reward',row:0,col:2,value:1});const m=new TurnManager(s,DEFAULT_RULES);m.resolveProjectile({remainingDamage:64,lane:0,position:0});expect(m.state.birthSlot).toBe(1);expect(m.state.enemies[0].hp).toBe(99);expect(m.state.rewardBalls).toEqual([]); });
  it('matches the observed 32 -> reward 4 -> enemy 99 interaction', () => { const s=emptyState();s.birthSlot=null;s.plants[0][0]=plant(32);s.enemies.push({id:'enemy',row:0,col:3,width:1,height:1,hp:99,maxHp:99});s.rewardBalls.push({id:'reward',row:0,col:2,value:4});const m=new TurnManager(s,DEFAULT_RULES);m.perform({from:{row:0,col:0},to:{row:0,col:1}});expect(m.state.birthSlot).toBe(4);expect(m.state.enemies[0].hp).toBe(75); });
  it('emits reward hit, capture, and birth-slot update in lifecycle order', () => { const s=emptyState();s.rewardBalls.push({id:'reward',row:0,col:2,value:4});const m=new TurnManager(s,DEFAULT_RULES);m.resolveProjectile({remainingDamage:8,lane:0,position:0});expect(m.state.events.map(e=>e.type)).toEqual(['shot','reward-hit','reward-captured','spawn-slot-updated']);expect(m.state.events.at(-1)).toMatchObject({value:4}); });
  it('keeps plants independent: two 32 plants make eight 8-damage projectiles', () => { const s=emptyState();s.plants[0][0]=plant(32);s.plants[0][1]=plant(32);s.plants[1][0]=plant(1);s.enemies.push({id:'enemy',row:0,col:3,width:1,height:1,hp:99,maxHp:99});s.rewardBalls.push({id:'reward',row:0,col:2,value:4});const m=new TurnManager(s,DEFAULT_RULES);m.perform({from:{row:1,col:0},to:{row:1,col:1}});expect(m.state.enemies[0].hp).toBe(43); });
  ...
  it('keeps the birth slot when a capture value is lower (max register, HANDOFF 22)', () => { const s=emptyState();s.birthSlot=8;s.plants[0][0]=plant(4);s.rewardBalls.push({id:'low',row:0,col:2,value:1});const m=new TurnManager(s,DEFAULT_RULES);m.resolveProjectile({remainingDamage:64,lane:0,position:0});expect(m.state.birthSlot).toBe(8); });
  it('birth slot takes the max of multiple captures within one turn (HANDOFF 22)', () => { const s=emptyState();s.birthSlot=null;s.plants[0][0]=plant(8);s.rewardBalls.push({id:'r1',row:0,col:2,value:1},{id:'r2',row:0,col:3,value:4},{id:'r3',row:0,col:4,value:2},{id:'r4',row:0,col:5,value:8});const m=new TurnManager(s,DEFAULT_RULES);m.perform({from:{row:0,col:0},to:{row:0,col:1}});expect(m.state.birthSlot).toBe(8); });
  it('equal values do not auto-merge in the birth slot (HANDOFF 22)', () => { const s=emptyState();s.birthSlot=4;s.plants[0][0]=plant(4);s.rewardBalls.push({id:'eq',row:0,col:2,value:4});const m=new TurnManager(s,DEFAULT_RULES);m.resolveProjectile({remainingDamage:64,lane:0,position:0});expect(m.state.birthSlot).toBe(4); });
  ...
  it('natural reward spawns stay within the confirmed pool {1,2,4,8} (HANDOFF 20)', () => { const s=emptyState();s.birthSlot=2;s.plants[0][0]=plant(2);const m=new TurnManager(s,{...DEFAULT_RULES,rewardSpawning:true,rewardSpawnChance:1},()=>0.5);m.perform({from:'birth',to:{row:0,col:0}});if(m.state.rewardBalls.length){expect([1,2,4,8]).toContain(m.state.rewardBalls[0].value);} });
  ...
  it('spawns a new enemy and its reward only after old enemies advance', () => { const s=emptyState();s.plants[1][0]=plant(1);s.enemies.push({id:'old',row:0,col:5,width:1,height:1,hp:20,maxHp:20});const m=new TurnManager(s,{...DEFAULT_RULES,automaticEnemySpawning:true,enemySpawnChance:1,rewardSpawning:true,rewardSpawnChance:1,hardPressureCap:100},()=>0,'baseline');m.perform({from:{row:1,col:0},to:{row:1,col:1}});const advanced=m.state.events.findIndex(e=>e.type==='advance');const spawned=m.state.events.findIndex(e=>e.type==='enemy-spawned');const reward=m.state.events.findIndex(e=>e.type==='reward-spawned');expect(advanced).toBeGreaterThan(-1);expect(spawned).toBeGreaterThan(advanced);expect(reward).toBeGreaterThan(spawned);expect(m.state.enemies.find(e=>e.id!=='old')).toMatchObject({col:9,width:1,height:1}); });
  it('spawns reward balls at Column10 and advances only pre-existing balls', () => { const s=emptyState();s.plants[1][0]=plant(1);s.rewardBalls.push({id:'old-ball',row:4,col:3,value:1});const m=new TurnManager(s,{...DEFAULT_RULES,automaticEnemySpawning:true,enemySpawnChance:1,rewardSpawning:true,rewardSpawnChance:1},()=>0);m.perform({from:{row:1,col:0},to:{row:1,col:1}});expect(m.state.rewardBalls.find(ball=>ball.id==='old-ball')?.col).toBe(2);const newBall=m.state.rewardBalls.find(ball=>ball.id!=='old-ball');expect(newBall?.col).toBe(9);expect(m.state.events.findIndex(e=>e.type==='reward-advance')).toBeLessThan(m.state.events.findIndex(e=>e.type==='reward-spawned')); });
  ...
  it('records reward capture by value using the generated-reward ledger', () => { const s=emptyState();s.plants[0][0]=plant(4);const m=new TurnManager(s,{...DEFAULT_RULES,automaticEnemySpawning:false,rewardSpawning:false},()=>0,'baseline');m.debugSpawnReward(4);m.perform({from:{row:0,col:0},to:{row:0,col:1}});expect(m.state.metrics.at(-1)).toMatchObject({rollingRewardSpawnValue:4,rollingRewardCapturedValue:4,rewardCaptureRate:1}); });
  ...
  it('keeps reward balls out of every cell occupied by a 2x2 enemy', () => { const m=new TurnManager(emptyState(),DEFAULT_RULES,()=>0);m.state.enemies.push({id:'large',row:0,col:8,width:2,height:2,hp:4,maxHp:4});expect(m.debugSpawnReward(1)).toBe(true);const reward=m.state.rewardBalls[0];expect(reward.row).toBeGreaterThanOrEqual(2);expect(reward.col).toBe(9); });
